 ...
 **************************************************/
void smaug5_poly_add(poly *r, const poly *a, const poly *b) {
    unsigned int i;
    for (i = 0; i < LWE_N; i++)
 ...
 *            - poly *b: pointer to second input polynomial
 **************************************************/
void smaug5_poly_sub(poly *r, const poly *a, const poly *b) {
    unsigned int i;
    for (i = 0; i < LWE_N; i++)
        r->coeffs[i] = a->coeffs[i] - b->coeffs[i];
}
...
void matrix_vec_mult_sub(polyvec *r, const polyvec a[MODULE_RANK],
    ...
    for (i = 0; i < MODULE_RANK; ++i) {
        ...
        smaug5_poly_sub(&r->vec[i], &r->vec[i], &res);
    }
}
