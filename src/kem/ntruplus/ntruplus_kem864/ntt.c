	...
	for(int step = 24; step >= 3; step >>= 1)
	{
		for(int start = 0; start < NTRUPLUS_N; start += (step << 1))
		{
			...
			for(int i = start; i < start + step; i++)
			{
				...
				r[i + step] = ntruplus_barrett_reduce(r[i] - t1);
				r[i       ] = ntruplus_barrett_reduce(r[i] + t1);
			}
		}
	}
}
	...
	for(int step = 3; step <= 24; step <<= 1)
	{
		for(int start = 0; start < NTRUPLUS_N; start += (step << 1))
		{
			...
			for(int i = start; i < start + step; i++)
			{
				...
				r[i + step] = ntruplus_barrett_reduce(fqmul(zeta1, t1 - r[i]));
				r[i       ] = ntruplus_barrett_reduce(r[i] + t1);
			}
		}
	}
	...
	for(int step = 48; step <= NTRUPLUS_N/6; step = 3*step)
	{
		for(int start = 0; start < NTRUPLUS_N; start += 3*step)
		{
			...
			for(int i = start; i < start + step; i++)
			{
				...
				r[i         ] = ntruplus_barrett_reduce(r[i] + r[i + step] + r[i + 2*step]);
				r[i +   step] = t2;			
				r[i + 2*step] = t3;
			}			
		}
