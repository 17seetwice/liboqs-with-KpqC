...
int generate_matrix_LU(GF matrix_A[AIM2_NUM_INPUT_SBOX][AIM2_NUM_BITS_FIELD],
                        GF vector_b,
                        const uint8_t iv[AIM2_IV_SIZE])
{
  // Use heap allocation to avoid stack overflow
  GF (*matrix_L)[AIM2_NUM_BITS_FIELD] = malloc(AIM2_NUM_INPUT_SBOX * AIM2_NUM_BITS_FIELD * sizeof(GF));
  GF (*matrix_U)[AIM2_NUM_BITS_FIELD] = malloc(AIM2_NUM_INPUT_SBOX * AIM2_NUM_BITS_FIELD * sizeof(GF));

  if (!matrix_L || !matrix_U) {
    free(matrix_L);
    free(matrix_U);
...
int aim2(uint8_t ct[AIM2_NUM_BYTES_FIELD],
