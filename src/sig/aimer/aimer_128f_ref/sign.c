...
//int O void(i dk)
// committing to the seeds and the execution views of the parties (heap version)
static int run_phase_1_heap(signature_t *sign,
                      uint8_t (*commits)[AIMER_N][AIMER_COMMIT_SIZE],
                      uint8_t (*nodes)[2 * AIMER_N - 1][AIMER_SEED_SIZE],
                      mult_chk_t (*mult_chk)[AIMER_N],
                      ...
                      const uint8_t *sk, const uint8_t *m, size_t mlen)
{
  ...
  GF (*matrix_A)[AIM2_NUM_BITS_FIELD] = malloc(AIMER_L * AIM2_NUM_BITS_FIELD * sizeof(GF));
  GF vector_b = {0,};
  ...
  if (generate_matrix_LU(matrix_A, vector_b, sk + AIM2_NUM_BYTES_FIELD) != 0) {
    free(matrix_A);
    return -1;
  }
...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk)
{
  ...
  if (aim2(pk + AIM2_IV_SIZE, sk, pk) != 0) {
    return -1;
  }
        ...
        const uint8_t *sk)
{
  ...
  // Use heap allocation to avoid large stack usage
  uint8_t (*nodes)[2 * AIMER_N - 1][AIMER_SEED_SIZE] = malloc(AIMER_T * (2 * AIMER_N - 1) * AIMER_SEED_SIZE);
  uint8_t (*commits)[AIMER_N][AIMER_COMMIT_SIZE] = malloc(AIMER_T * AIMER_N * AIMER_COMMIT_SIZE);
  mult_chk_t (*mult_chk)[AIMER_N] = malloc(AIMER_T * AIMER_N * sizeof(mult_chk_t));
  GF (*alpha_v_shares)[2][AIMER_N] = malloc(AIMER_T * 2 * AIMER_N * sizeof(GF));

  if (!nodes || !commits || !mult_chk || !alpha_v_shares) {
  ...
  if (run_phase_1_heap(sign, commits, nodes, mult_chk, alpha_v_shares, sk, m, mlen) != 0) {
        ...
        const uint8_t *pk)
{
  ...
  if (generate_matrix_LU(matrix_A, vector_b, pk) != 0) {
    free(matrix_A);
    return -1;
  }
