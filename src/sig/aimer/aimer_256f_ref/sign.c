        ...
        const uint8_t *sk)
{
  ...
  // Use heap allocation to avoid stack overflow (~497KB total)
  // Allocate as flat arrays and cast to proper dimensions  
  uint8_t *nodes_flat = malloc(AIMER_T * (2 * AIMER_N - 1) * AIMER_SEED_SIZE);
  uint8_t *commits_flat = malloc(AIMER_T * AIMER_N * AIMER_COMMIT_SIZE);
  mult_chk_t *mult_chk_flat = malloc(AIMER_T * AIMER_N * sizeof(mult_chk_t));
  GF *alpha_v_shares_flat = malloc(AIMER_T * 2 * AIMER_N * sizeof(GF));
  
  if (!nodes_flat || !commits_flat || !mult_chk_flat || !alpha_v_shares_flat) {
    free(nodes_flat);
    free(commits_flat);
    free(mult_chk_flat);
    free(alpha_v_shares_flat);
    return -1; // Memory allocation failed
  }
  
  // Cast to multi-dimensional arrays for compatibility
  uint8_t (*nodes)[2 * AIMER_N - 1][AIMER_SEED_SIZE] = (uint8_t (*)[2 * AIMER_N - 1][AIMER_SEED_SIZE])nodes_flat;
  uint8_t (*commits)[AIMER_N][AIMER_COMMIT_SIZE] = (uint8_t (*)[AIMER_N][AIMER_COMMIT_SIZE])commits_flat;
  mult_chk_t (*mult_chk)[AIMER_N] = (mult_chk_t (*)[AIMER_N])mult_chk_flat;
  GF (*alpha_v_shares)[2][AIMER_N] = (GF (*)[2][AIMER_N])alpha_v_shares_flat;
