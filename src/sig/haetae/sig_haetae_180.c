...
OQS_SIG *OQS_SIG_haetae_180_new(void) {
	...
	sig->method_name = OQS_SIG_alg_haetae_180;
	...
	sig->length_public_key = OQS_SIG_haetae_180_length_public_key;
	sig->length_secret_key = OQS_SIG_haetae_180_length_secret_key;
	sig->length_signature = OQS_SIG_haetae_180_length_signature;

	sig->keypair = OQS_SIG_haetae_180_keypair;
	sig->sign = OQS_SIG_haetae_180_sign;
	sig->verify = OQS_SIG_haetae_180_verify;
	sig->sign_with_ctx_str = OQS_SIG_haetae_180_sign_with_ctx_str;
	sig->verify_with_ctx_str = OQS_SIG_haetae_180_verify_with_ctx_str;
...
OQS_API OQS_STATUS OQS_SIG_haetae_180_sign_with_ctx_str(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *ctx, size_t ctxlen, const uint8_t *secret_key){
	...
	if (ctx_str == NULL && ctx_str_len == 0) {
		return OQS_SIG_haetae_180_sign(signature, signature_len, message, message_len, secret_key);
	} else {
		return OQS_ERROR;
	}	
}
...
OQS_API OQS_STATUS OQS_SIG_haetae_180_verify_with_ctx_str(const uint8_t *message, size_t message_len, const uint8_t *signature, size_t signature_len, const uint8_t *ctx, size_t ctxlen, const uint8_t *public_key){}
	...
	if (ctx_str == NULL && ctx_str_len == 0) {
		return OQS_SIG_haetae_180_verify(message, message_len, signature, signature_len, public_key);
	} else {
		return OQS_ERROR;
	}
#endif